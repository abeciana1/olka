...
export async function getLandingPage(slug, lang = 'en', preview = 0) {
  try {
    const params = {
      locale: lang,
      preview: preview
    }
    const page = await butter.page.retrieve(
      'landing-page',
      slug,
      params
    );
...
export async function getMainMenu() {
  try {
    ...
    console.log('main fetch', mainMenu)
    return mainMenu ? {
      logo: mainMenu.logo,
      mainMenu: mainMenu.menu_items
    } : []
  }
  catch (e) {
    throw e.response.data.detail
